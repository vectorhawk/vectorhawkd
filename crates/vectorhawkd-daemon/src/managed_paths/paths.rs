...
/// Resolve `~/.agents/skills/` — the canonical managed-skill location.
pub fn agents_skills_dir() -> Option<PathBuf> {
    vectorhawkd_mcp::ownership::agents_skills_dir()
}

/// Resolve `~/.claude/skills/` — where Claude Code looks. VectorHawk places
/// only symlinks here, pointing at `agents_skills_dir()`.
pub fn claude_skills_link_dir() -> Option<PathBuf> {
...
mod tests {
    ...
    fn path_helpers_return_some() {
        ...
        assert!(agents_skills_dir().is_some());
        assert!(claude_skills_link_dir().is_some());
